...
fn main() {
    let value = Arc::new(Mutex::new(0)); // ✅ Use Arc<Mutex<T>> which is Sync + Send

    let value_clone = Arc::clone(&value);
    let handle = thread::spawn(move || {
        let mut num = value_clone.lock().unwrap(); // ✅ Lock for mutation
        *num += 100;
    });

    handle.join().unwrap();

    println!("Final result: {}", *value.lock().unwrap()); // ✅ Output: 100
}
